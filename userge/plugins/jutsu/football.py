...
async def fb_teams_(message: Message):
    ...
    team_list = f"<b>Teams</b> competing in <b>{lname} <i>({nation})</i> ({league_})</b> this season <i>({start_end})</i>:" + " [<b>{}</b>]<br>"
    total_ = 0
    try:
    ...
    await message.edit(f"Teams in <b>{lname}</b> for season <i>({start_end})</i> are <a href='{link_}'><b>HERE</b></a>")
...
async def fb_sched_(message: Message):
    ...
    matches_sch = f"Matches for <b>{the_team}</b> this season <i>({start_end})</i>:<br><br>"
...
async def fb_fixtures_(message: Message):
    ...
    out_ = (
        f"<b>LEAGUE:</b> <i>{league_} ({country}) {start_end}</i><br><br>"
    )
    sr_ = 1
    ...
    await message.edit(f"Fixtures for {league_} this season <i>({start_end})</i> is <a href='{link_}'><b>HERE</b></a>")
...
async def fb_stand_(message: Message):
    ...
    try:
        connection = http.client.HTTPConnection('api.football-data.org') 
        headers = {'X-Auth-Token': FOOTBALL_API}
        connection.request('GET', f'/v2/competitions/{league_}/standings', None, headers)
        response = json.loads(connection.getresponse().read().decode())
    except Exception as e:
        await message.err(e, del_in=5)
        return
    ...
    if league_ == "CL" or league_ == "CLI":
        ...
        for tables in stand_:
            ...
            for one in g_table:
                ...
                point_ = one['points']
                standings += (
                    f"<b>{position:02}.</b> P-{played_:02} | W-{won_:02} | D-{draw_:02} | L-{lost_:02} | <b>{point_:02}</b> | {team_n}<br>"
                )
            standings += "<br>"
    else:
    ...
    await message.edit(f"Standing table of <b>{league_n}</b> this season <i>({start_end})</i> is <a href='{link_}'><b>HERE</b></a>")
