...
async def test_call(message: Message):
    up = "0 likes"
    down = "0 dislikes"
    await userge.bot.send_message(message.chat.id, "Voting for replied message.", reply_to_message_id=message.reply_to_message.message_id, reply_markup=buttons(up, down))
    ...
    try:
        ...
        if "up" in c_q.data:
            ...
            if tapper in votes_up:
                ...
                text_up = re.sub(r"\d+", f"{int(number) + 1}", text_up, count=1)
            await VOTE.update_one({'_id': f'{c_q.message.chat.id}_{vote_msg}'}, {'$set': {'up': votes_up}}, upsert=True)
            ...
            if tapper in votes_down:
                ...
                text_down = re.sub(r"\d+", f"{int(number) + 1}", text_down, count=1)
            await VOTE.update_one({'_id': f'{c_q.message.chat.id}_{vote_msg}'}, {'$set': {'down': votes_down}}, upsert=True)
        elif "list" in c_q.data:
            if c_q.from_user.id not in Config.OWNER_ID:
                return await c_q.answer("Only the bot owner can see this list.", show_alert=True)
        ...
        await c_q.edit_message_text("Thanks for the vote.", reply_markup=buttons(text_up, text_down))
